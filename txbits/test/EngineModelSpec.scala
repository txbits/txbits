...
class EngineModelSpec extends Specification with Mockito {
  ...
  "Engine" should {
    ...
    "update stats after a bid matches" in new WithCleanTestDbApplication {
      val asker = globals.userModel.create("test@test.test", "", false).get
      val bidder = globals.userModel.create("test2@test.test", "", false).get
      ...
      stats_res.head(2).value should be equalTo 1
      stats_res.head(3).value should be equalTo 1
      stats_res.head(4).value should be equalTo 1
      stats_res.head(5).value should be equalTo 1
    }
