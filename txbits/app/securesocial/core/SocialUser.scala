 ...
 */
case class SocialUser(id: Long, email: String, verification: Int, language: String, onMailingList: Boolean, TFAEnabled: Boolean = false, pgp: Option[String] = None, username: String = "")
  ...
  implicit def writes = new Writes[SocialUser] {
    def writes(u: SocialUser): JsValue = {
      ...
      Json.obj("email" -> u.email, "verification" -> u.verification, "onMailingList" -> u.onMailingList,
        "TFAEnabled" -> u.TFAEnabled, "pgp" -> u.pgp, "language" -> u.language, "username" -> u.username)
    }
  }
}
